...
## SNMPv2
- security and functional enhancements form SNMPv1
GetBulkRequest: Enable Manager to retrive large block of data efficiently
- informRequesrt: Enable one manager to send trap type of information to another
- new convention for creating and deleting rows in table (RMON inspired)

MAX access: specifies the access level for a managed object within a MIB module
- i.e. read write or create (row)

Every SNMP message contains a protocol data unit (PDU)

Row creation and deletion:
- instead of direct row creation and deletion -> embed the semantics of row creation and deletion into the MIB with new textual convention called RowStatus.
	- row creation and deletion is performed using SET and GET operations similar to RMON
	- Requirement: there must be a RowStatus columnar object with two methods:
		- createAndWait
			- SET: to notInService, row does not have any values / isnt fully initialized. if no default values thens set to notReady
			- GET: will return noSuchInstance or noSuchObject
			- SET: update any noSuchInstance objects and update row status to be active
		- CreateAndGo
			- just create and set status to active
	- Row Deletion
		- set row status to destroy

is NOT innately atomic
-  a variable-bindings list returns as many values as possible from the request

GetBulkRequest PDU
- possible to specify the multiple lexicographic succesors to be selected
- includes a list of (N + R) variable names in the variable bindings list
- goes Row by Row wheras GetNextRequest is column by column


## Network Security
Confidentiality: only sender and intended receiver must understand the message contents
- done through encryption (shared key/public key)
Integrity: the message is not altered during transit or afterwards without detection
- hashes
Authentication: send and receiver wnat to confirm identity of each other
- public key signatures

Hashing collision problem: two messages that have the same hash, when this problem is solved, must abandon hash function cause it may not be secure anymore
- best hash function is MD5
	- `md5sum a2.txt`
- SHA1
	- `sha1sum a2.txt`

OPENSSL
- provides implementation for many cryptographic algorithms, including symmetric encryption (like AES), asymmetric encryption (RSA), hash functions (SHA-256) and digital signatures
	- widely used to implement SSL (Secure Sockets Layer) and TLS (Transport Layer Security) protocols,
- `openssl genpkey -algorithm RSA -out private_key.pem`
- `openssl dgst -sha256 plaintext`
	- generates a hash text of plaintext

MAC (Message Authentication Code)
- a short array of bits, i,e, 256 bits, that authenticates a message
- mac value is generated by sender and reciever using the sent message
	- if mac value does not match that means the sent message has been altered

HMAC 
- hash based message authentication code 
	- like MAC but involves a hash function aswell
	- same idea with comparing mac values but instead compare the hash code

MAC vs MEssage Digest and Digital Signature
- Message Digest provides message integrity
- Digital Signature use asymmetric cryptography
	- Signer and verifier use different keys from the same keypair
	- Only the signer can produce the signature because only they posses the private key 
		- this means the digital signature cannot be denied (non repudiation)
			- ^ MAC CAN NOT DO THIS because both sneder and receiver use the same secret key in mac, meaning a digital signature using mac can reproduce by both parties
- MAC provides message integrity and authenticity
- An Attacker can modify the ciphertext ot produce a meaningful, intentional change in the resulting plaintext
	- this is why a HMAC authentication signature is included with each encrypted message to ensure that an attacker cannot modify messages and still have them appear valid 

Authenticated Encryption 

Encrypt then MAC (EtM) (Diffie-Hellman Key Exchange)
- generation:
	- encrypt plain text to obtain cipher text
	- hash ciphertext to obtain mac
	- sender ciphertext, MAC
- Verification 
	- compute MAC on cipehrtext
	- veryify computed MAC matches received MAC
	- Decrypt ciphertext to plaintext
- due to the difficulty of the discrete logarithm problem, it is computationally infeasible to determine the private key 
- WEAKNESS 
	- Man in the Middle Attacks
		- attacker intercepts and replaces the public keys, 
		- mitigate this risk by authenticating public keys through certificates or digital signatures

RSA (rivest, shanir, adelson algorithm)
- public key with OpenSSL
	- extract public key from keypair
		- `openssl pkey -in rsa-keypair-1.pem -pubout -out rsa-public-key-1.pem`
			- `-pubout` only the public key in output, not the whole keypair
- • Encrypt
	- • openssl pkeyutl -encrypt -in content -out content.encrypted -pubin -inkey rsa-public-key-1.pem
- • Decrypt
	- • openssl pkeyutl -decrypt -in content.encrypted -out content.decrypted -inkey rsa-keypair-1.pem
- Verify
	- `md5sum content`
- WHY IS RSA GOOD?
- suppose you know Bob's public key. How hard is it to determine d? essentially need to find factors of n without knowing the two
	- factors p and q
		- factoring a big number is hard
- RSA encryption is often used to securely exchange a symmetric session key between two parties, such as Alice and Bob, rather than encrypting all data exchanged between them
	- slow, DES or AES are a lot faster

Replay Attacks
- caused by not authenticating freshness or uniqueness of each interaction
- prevent by using challenge-response method or include a timestamp or nonce ( a unique random number for each session )

use Digital Certs to mitigate Man in the Middle attacks

Certification Authorities
- get public key through a certificate sent by Bob with its own public key

Digital Signature produced using a private key
- digital signature is applied to message digest
- known as Hash and Sign paradigm
	- message digest is signed instead because it is much smaller in size compared to the original method, signature algorithm is slow
	- RSA is the slowest but fastest to verify

MAC and Digital Signature
- provides both auth and integrity
- Digital Signature can only be created by the sender
	- non repudiation

X.509 cert binds to public key using a digital signature

Certificate Signing Chains
- ordered collection of certs where each cert is signed by the next certificate in the collection.
- leaf cert is self signed, all else are signed by the next cert
	- leaf calle Root CA certs
- Why?
	- distributes the burden of cert issuance 
	- compromissing the Root CA will undermine all certs
		- heavily secured 
- Purpose:
	- prevent Man in the Middle attacks

Certificate Pinning:
- Initial Conn: first time Alice connects to serverBob,receives serverBob's certficates and verrifies against a trusted Certficate Authority (CA)
	- during this, Alice's browser pins certain info:
		- the CA from serverBob
		- serverBob public key

SSH 
- establish encrypted communication channel between two hosts
- Client Server Key Exchange: negotiate a session key
- Trust on First Use (TOFU)
	- on first connection, prompted to trust the server, ssh client will save public key in a known_hosts file

TLS
- orginal and primary role
- browsers have embedded lists of certificate authorities, known as trust storesd, trusted by the browser vendow
